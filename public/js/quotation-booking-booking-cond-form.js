...
*/
	...
	function updateTotalAmount(){
		...
		if(propertysize)
			var propertysizeprice = propertysize.split('|')[1];
		else
		...
		if(!residentialpartprice)
			residentialpartprice = 0;
		
		if(!commertialpartprice)
			commertialpartprice = 0;
			
		var diningroom = $('input[name=diningroom]:checked').val();
		if(diningroom)
			var diningroomprice = diningroom.split('|')[1];
		else
			var diningroomprice = 0;
		
		var kitchen = $('input[name=kitchen]:checked').val();
		if(kitchen)
			var kitchenprice = kitchen.split('|')[1];
		else
			var kitchenprice = 0;
		
		var oven = $('input[name=oven]:checked').val();
		if(oven)
			var ovenprice = oven.split('|')[1];
		else
			var ovenprice = 0;
		...
		$('.validkitchenitems input[type=checkbox]:checked').each(function(){        
			...
			if(kivalues)
				var kiprice = parseFloat(kivalues.split('|')[1]);
...
*/	
	...
	function nextStep2() {
		...
		if( cleaningtype && propertyaddress.length > 0 && propertyaddress ) {
	...
	function nextStep3() {
		...
		if( servicetype == "Residential" ) {
			...
			if( bedrooms && bathrooms && bathtubs && sittingroom ) {
			...
			if( storerooms )
				$( ".validstorerooms .help-block.with-errors" ).html( '' );
			else
				$( ".validstorerooms .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Please Select Store Rooms</li></ul>' );
			...
			if( rooms && conferenceroom && storerooms && washrooms ) {
		...
		if( propertysize && servicetype && servicetypepropertychecked && diningroom && kitchen && oven && kitchenitems ) {
